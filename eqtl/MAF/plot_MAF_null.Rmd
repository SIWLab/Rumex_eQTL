...
set.seed(123)
ML_eqtl_sig_random <- ML_eqtl_sig %>% group_by(phenotype_id) %>% slice_sample(n = 1) # 10671,8237
ML_eqtl_sig_random_af <- ML_eqtl_sig_random %>% dplyr::select(variant_id, maf) 
ML_eqtl_sig_random_af <- ML_eqtl_sig_random_af %>% group_by(variant_id) %>% mutate(count = n()) %>% ungroup() %>% filter(count == 1) # 10374, 7944
...
number of real eqtls 10374
number of fake eqtls
...
FL_eqtl <- rbind(FL_A1, FL_A2, FL_A3, FL_A4) # 1272993
FL_eqtl_sig <- FL_eqtl %>% filter(pval_nominal <= 0.048762071) # 99992
FL_eqtl_sig <- FL_eqtl_sig %>% mutate(maf = ifelse(af <= 0.5, af, 1 - af))
FL_eqtl_sig <- FL_eqtl_sig %>% group_by(phenotype_id) %>% mutate(count = n()) %>% ungroup() %>% filter(count <= 10) # 28780

set.seed(123)
FL_eqtl_sig_random <- FL_eqtl_sig %>% group_by(phenotype_id) %>% slice_sample(n = 1) # 10671,8237
FL_eqtl_sig_random_af <- FL_eqtl_sig_random %>% dplyr::select(variant_id, maf) 
FL_eqtl_sig_random_af <- FL_eqtl_sig_random_af %>% group_by(variant_id) %>% mutate(count = n()) %>% ungroup() %>% filter(count == 1) # 10374, 7944
...
MP_eqtl <- rbind(MP_A1, MP_A2, MP_A3, MP_A4) # 1272993
MP_eqtl_sig <- MP_eqtl %>% filter(pval_nominal <= 0.0160989562) # 99992
MP_eqtl_sig <- MP_eqtl_sig %>% mutate(maf = ifelse(af <= 0.5, af, 1 - af))
MP_eqtl_sig <- MP_eqtl_sig %>% group_by(phenotype_id) %>% mutate(count = n()) %>% ungroup() %>% filter(count <= 10) # 28780

set.seed(123)
MP_eqtl_sig_random <- MP_eqtl_sig %>% group_by(phenotype_id) %>% slice_sample(n = 1) # 10671,8237
MP_eqtl_sig_random_af <- MP_eqtl_sig_random %>% dplyr::select(variant_id, maf) 
MP_eqtl_sig_random_af <- MP_eqtl_sig_random_af %>% group_by(variant_id) %>% mutate(count = n()) %>% ungroup() %>% filter(count == 1) # 10374, 7944
