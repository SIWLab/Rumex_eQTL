...
ML_eqtl_sig <- ML_eqtl_full %>% filter(pval_nominal < pval_nominal_threshold) # 17283

FL_eqtl_full <- inner_join(FL_eqtl, FL[,c(1,16:19)], by = "phenotype_id") 
FL_eqtl_sig <- FL_eqtl_full %>% filter(pval_nominal < pval_nominal_threshold) # 35202

MP_eqtl_full <- inner_join(MP_eqtl, MP[,c(1,16:19)], by = "phenotype_id") 
MP_eqtl_sig <- MP_eqtl_full %>% filter(pval_nominal < pval_nominal_threshold) # 9835
```
...
ML_eqtl_sig <-read.csv(paste0(data, "ML_eqtl_sig.csv"))17283


length(unique(ML_eqtl_sig$phenotype_id)) 
length(unique(FL_eqtl_sig$phenotype_id)) 
length(unique(MP_eqtl_sig$phenotype_id)) 
