...
# extract the off diagonal eqtls in the lifestage comparison
```{r}
...
colnames(ml_mp)[3:4] <- c("male_leaf", "pollen") # 130, 201

ml_mp_inter1 <- ml_mp %>% filter(male_leaf > 0 & pollen < 0) # 2, 7
ml_mp_inter2 <- ml_mp %>% filter(male_leaf < 0 & pollen > 0) # 3, 7
ml_mp_inter <- rbind(ml_mp_inter1, ml_mp_inter2) # 14
```
...
colnames(fl_mp)[3:4] <- c("female_leaf", "pollen") # 53, 72

fl_mp_inter1 <- fl_mp %>% filter(female_leaf > 0 & pollen < 0) # 2, 4
fl_mp_inter2 <- fl_mp %>% filter(female_leaf < 0 & pollen > 0) # 3
fl_mp_inter <- rbind(fl_mp_inter1, fl_mp_inter2) # 5, 7
```
...
colnames(mp_ml)[3:4] <- c("pollen", "male_leaf") # 138, 211
mp_fl <- inner_join(mp, fl, by = c("phenotype_id","variant_id"))
colnames(mp_fl)[3:4] <- c("pollen", "female_leaf") # 84, 114

mp_ml_inter1 <- mp_ml %>% filter(male_leaf < 0 & pollen > 0) # 3, 9
mp_ml_inter2 <- mp_ml %>% filter(male_leaf > 0 & pollen < 0) # 2, 9
mp_ml_inter <- rbind(mp_ml_inter1, mp_ml_inter2) # 5, 18

mp_fl_inter1 <- mp_fl %>% filter(female_leaf < 0 & pollen > 0) # 2
mp_fl_inter2 <- mp_fl %>% filter(female_leaf > 0 & pollen < 0) # 2
mp_fl_inter <- rbind(mp_fl_inter1, mp_fl_inter2) # 4
```
...
write.csv(lifestage_inter, file = paste0(data, "lifetage_discordance_full_Nov.csv"), row.names = F, quote = F)
lifestage_inter<- read.csv(paste0(data, "lifetage_discordance_full_Nov.csv"))

write.csv(mp_fl_inter, file = paste0(data, "mp_fl_inter_Nov.csv"), row.names = F, quote = F) # 4, 10
write.csv(fl_mp_inter, file = paste0(data, "fl_mp_inter_Nov.csv"), row.names = F, quote = F) # 5, 7
```

```{r}
ml_mp1 <- lifestage_inter %>% filter(male_leaf < 0 & pollen > 0) # 11
ml_mp2 <- lifestage_inter %>% filter(male_leaf > 0 & pollen < 0) # 12
range(ml_mp1$maf)
median(ml_mp1$maf)
range(ml_mp2$maf)
median(ml_mp2$maf)
lifestage_inter <- left_join(lifestage_inter, ML_eqtl_sig, by = c("phenotype_id", "variant_id"))
```
