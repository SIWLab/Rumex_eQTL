...
```{r}
pl_egene <- pl %>% filter(qval < 0.1) # 2031, 1566
...
```{r}
pl_eqtl_full <- inner_join(pl_eqtl, pl_egene[,c(1,16:19)], by = "phenotype_id") # 223329, 173054, 176402
pl_eqtl_sig <- pl_eqtl_full %>% filter(pval_nominal < pval_nominal_threshold) # 12978, 9754, 9873
...
```{r}
nrow(pl_eqtl_sig[pl_eqtl_sig$slope > 0,]) # 5481, 6087, 6173
nrow(pl_eqtl_sig[pl_eqtl_sig$slope < 0,]) # 7497, 3667, 3700
...
```{r}
ml_overlap <- inner_join(pl_eqtl_sig, ML_eqtl_sig, by = c("phenotype_id", "variant_id")) # 4535
mp_overlap <- inner_join(pl_eqtl_sig, MP_eqtl_sig, by = c("phenotype_id", "variant_id")) # 4964

no_overlap <- pl_eqtl_sig %>% anti_join(ML_eqtl_sig, by = c("phenotype_id", "variant_id")) %>% anti_join(MP_eqtl_sig, by = c("phenotype_id", "variant_id")) # 1142
...
overlap_eqtl <- pl %>% inner_join(ml, by = c("phenotype_id", "variant_id")) %>% inner_join(mp, by = c("phenotype_id", "variant_id")) # 768

# different degree
sum(overlap_eqtl$pl > 0 & overlap_eqtl$ml > 0 & overlap_eqtl$mp > 0) # 422
...
sum(overlap_eqtl$pl < 0 & overlap_eqtl$ml < 0 & overlap_eqtl$mp < 0) # 204
sum(overlap_eqtl$pl < 0 & overlap_eqtl$ml > 0 & overlap_eqtl$mp < 0) # 10
sum(overlap_eqtl$pl < 0 & overlap_eqtl$ml < 0 & overlap_eqtl$mp > 0) # 0
