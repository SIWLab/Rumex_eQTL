...
```{r}
ML <- read.table(paste0(data, "ML.cis_qtl.txt"), header = T) # 14792
...
ML 14658 tested, 4075 (q < 0.1)
FL 14733 tested, 4260 (q < 0.1)
MP 13794 tested, 2192 (q < 0.1)

## extract eGenes
```{r}
ML_egene <- ML %>% filter(qval < 0.1) # 2560
FL_egene <- FL %>% filter(qval < 0.1) # 3459
MP_egene <- MP %>% filter(qval < 0.1) # 1474
# Venn of egenes
...
ML_eqtl <- rbind(ML_A1, ML_A2, ML_A3, ML_A4) # 1369956
sum(ML$num_var) # 1369956
...
ML_eqtl_sig <- ML_eqtl_full %>% filter(pval_nominal < pval_nominal_threshold) # 11882

FL_eqtl_full <- inner_join(FL_eqtl, FL[,c(1,16:19)], by = "phenotype_id") 
FL_eqtl_sig <- FL_eqtl_full %>% filter(pval_nominal < pval_nominal_threshold) # 23944

MP_eqtl_full <- inner_join(MP_eqtl, MP[,c(1,16:19)], by = "phenotype_id") 
MP_eqtl_sig <- MP_eqtl_full %>% filter(pval_nominal < pval_nominal_threshold) # 6464
```
...
length(unique(ML_eqtl_sig$phenotype_id)) # 1891
length(unique(FL_eqtl_sig$phenotype_id)) # 2692
length(unique(MP_eqtl_sig$phenotype_id)) # 1158
