...
# 
only genotype term with sex as a covariate
sex_eqtl_sig
sex_eqtl_sig_min

only genotype term without sex as a covariate
nosex_eqtl_sig
nosex_eqtl_sig_min
...
```{r}
sex_interaction_A1 <- read_parquet(paste0(data,"nosex_inter.cis_qtl_pairs.A1.parquet"))
sex_interaction_A2 <- read_parquet(paste0(data,"nosex_inter.cis_qtl_pairs.A2.parquet"))
sex_interaction_A3 <- read_parquet(paste0(data,"nosex_inter.cis_qtl_pairs.A3.parquet"))
sex_interaction_A4 <- read_parquet(paste0(data,"nosex_inter.cis_qtl_pairs.A4.parquet"))
colnames(sex_interaction_A4)

sex_inter <- rbind(sex_interaction_A1, sex_interaction_A2, sex_interaction_A3, sex_interaction_A4) # 1063500
colnames(sex_inter)
```
...
```{r}
sex_inter_top <- read.table(paste0(data, "nosex_inter.cis_qtl_top_assoc.txt"), header = T) # 14374
...
227 2723
```{r}
g <- p_g_adj_01 %>% select(variant_id, af) %>% distinct() # 2950, 7273
gi <- p_gi_adj_01 %>% select(variant_id, af)  %>% distinct() # 233, 9163
...
p8 <- ggplot(p_data, aes(x=x, y=y_prop)) +
  geom_bar(stat="identity", width = 0.01, color="white")  + theme_classic() + ggtitle("Pollen")+ labs(x="MAF", y="Proportion", title="G")+ theme(text = element_text(size = 10)) + ylim(0, 0.23)+ geom_vline(xintercept=0.0807692, color = "red",linetype="dashed")
p7 + p8
```
