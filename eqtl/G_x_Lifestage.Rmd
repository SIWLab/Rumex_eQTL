...
colnames(ml_mp)[3:4] <- c("male_leaf", "pollen") # 130

ml_mp_inter1 <- ml_mp %>% filter(male_leaf > 0 & pollen < 0) # 2
ml_mp_inter2 <- ml_mp %>% filter(male_leaf < 0 & pollen > 0) # 3
...
colnames(mp_ml)[3:4] <- c("pollen", "male_leaf") # 138
mp_fl <- inner_join(mp, fl, by = c("phenotype_id","variant_id"))
colnames(mp_fl)[3:4] <- c("pollen", "female_leaf") # 84

mp_ml_inter1 <- mp_ml %>% filter(male_leaf < 0 & pollen > 0) # 3
mp_ml_inter2 <- mp_ml %>% filter(male_leaf > 0 & pollen < 0) # 2
mp_ml_inter <- rbind(mp_ml_inter1, mp_ml_inter2) # 5
...
write.csv(lifestage_inter, file = paste0(data, "lifetage_discordance_full.csv"), row.names = F, quote = F)
write.csv(mp_fl_inter, file = paste0(data, "mp_fl_inter.csv"), row.names = F, quote = F) # 4
write.csv(fl_mp_inter, file = paste0(data, "fl_mp_inter.csv"), row.names = F, quote = F) # 5
```
