...
need to generate ML_eqtl_sig_min from eqtl_MAF.Rmd
```{r}
...
colnames(pair1)[3:5] <- c("male_leaf", "female_leaf", "sig_fl") # 1042, 1548
...
colnames(pair2)[3:5] <- c("female_leaf", "male_leaf", "sig_ml") # 1090, 1509
pair2$sig_fl <- "Significant"

ml_fl <- rbind(pair1, pair2) # 2132, 3057
...
colnames(pair1)[3:5] <- c("male_leaf", "pollen", "sig_mp") # 1297, 2301
pair1$sig_ml <- "Significant"

ml <- ML_eqtl_full %>% dplyr::select(phenotype_id, variant_id, slope, category)
mp <- MP_eqtl_sig_min %>% dplyr::select(phenotype_id, variant_id, slope)
pair2 <- inner_join(mp, ml, by = c("phenotype_id","variant_id"))
colnames(pair2)[3:5] <- c("pollen", "male_leaf", "sig_ml") # 935, 1503
pair2$sig_mp <- "Significant"

ml_mp <- rbind(pair1, pair2) # 2232, 3804
...
# write output
```{r}
write.csv(ml_fl, file = paste0(data, "effect_size_ml_fl_Nov.csv"), row.names = F, quote = F)
write.csv(ml_mp, file = paste0(data, "effect_size_ml_mp_Nov.csv"), row.names = F, quote = F)
ml_fl <- read.csv(paste0(data, "effect_size_ml_fl_Nov.csv"))
ml_mp <- read.csv(paste0(data, "effect_size_ml_mp_Nov.csv"))

ml_mp <- ml_mp[!duplicated(ml_mp), ] # 3804, 3695
ml_fl <- ml_fl[!duplicated(ml_fl), ] # 3057, 2907
```


```{r}
ml_mp <- ml_mp %>% filter(category == "both")

ml_mp1 <- ml_mp %>% filter(male_leaf < 0 & pollen > 0) # 16
ml_mp2 <- ml_mp %>% filter(male_leaf > 0 & pollen < 0) # 16
