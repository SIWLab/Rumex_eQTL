...
# MP
cnt <- read.table("normalized_counts_mpn_auto.bed", header = T, check.names = F)
col_names <- colnames(cnt)
...
colnames(cnt_shuffled) <- col_names
colnames(cnt_shuffled)[1] <- "#chr"
write.table(cnt_shuffled, paste0("normalized_counts_mpn_auto_", i, ".bed"), sep = "\t", row.names = FALSE, quote = FALSE)
