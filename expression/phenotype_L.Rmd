...
readcnt_leaf <- readcnt_leaf %>% filter(chrom_new != "X" & chrom_new != "Y") # 19486 -> 16124
...
readcnt_matrix<-as.matrix(readcnt_leaf[ , -c(1:5,151,152)]) 
...
normalized_countsl <- normalized_countsl[,c(146,1:145)]
...
normalized_counts_ln_auto <- normalized_counts_ln %>% filter(chrom != "PAR") # 14985
