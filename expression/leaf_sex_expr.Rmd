...
readcnt_leaf <- readcnt_leaf %>%  mutate(m_mean = rowMeans(readcnt_leaf[,c(2:72)] )) %>% mutate(f_mean = rowMeans(readcnt_leaf[,c(73:146)] ))
# remove genes with low read counts
readcnt_leaf <- readcnt_leaf %>% filter(m_mean >= 5 | f_mean >= 5) # 37659 -> 19486

readcnt_matrix<-as.matrix(readcnt_leaf[ , -c(1,147,148)]) 
...
head(DE_sex) 
DE_sex <- DE_sex %>% filter(padj<0.05) # 5606
...
DE_sex_m <-  DE_sex %>% filter(log2FoldChange > 0 & padj < 0.05) # 
DE_sex_f <-  DE_sex %>% filter(log2FoldChange < 0 & padj < 0.05) # 

DE_sex_m <-  DE_sex %>% filter(log2FoldChange > 1 & padj < 0.05) # 
DE_sex_f <-  DE_sex %>% filter(log2FoldChange < -1 & padj < 0.05) # 
977+298

DE_sex_m <-  DE_sex %>% filter(log2FoldChange > 2 & padj < 0.05) # 
DE_sex_f <-  DE_sex %>% filter(log2FoldChange < -2 & padj < 0.05) # 
...
all genes, FC = 2
```{r}
DE_sex_m <-  DE_sex %>% filter(log2FoldChange > 1 & padj < 0.05) # 1087
DE_sex_f <-  DE_sex %>% filter(log2FoldChange < -1 & padj < 0.05) # 412
...
all genes, FC = 1
```{r}
DE_sex_m <-  DE_sex %>% filter(log2FoldChange > 0 & padj < 0.05) # 3308
DE_sex_f <-  DE_sex %>% filter(log2FoldChange < 0 & padj < 0.05) # 2298
...
```{r}
data <- matrix(c(1087, 412, 296, 73), nrow = 2, byrow = TRUE)
