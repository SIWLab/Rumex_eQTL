...
```{r}
pl_eqtl_sig <- read.csv(paste0(data, "pl_eqtl_sig.csv")) # 12978
interesting_genes <- unique(pl_eqtl_sig$phenotype_id) # 2029
...
results <- result_table(resultFisher, GOdata)
results <- results %>% filter(fisher_pval < 0.05)
write.csv(results, file=paste0(data, "GO_diff_pl.csv"),row.names = FALSE, quote=FALSE )
```
